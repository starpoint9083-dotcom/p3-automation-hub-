...
for(const guard of ["HttpOnly","SameSite=Strict","Secure","CSRF_REJECTED","AUTH_REQUIRED","ADMIN_PASSWORD"])if(!auth.includes(guard))throw new Error(`MISSING_AUTH_CONTROL:${guard}`);
...
console.log(JSON.stringify({ok:true,version:pkg.version,required_files:required.length,routes:routes.length,wrangler:pkg.devDependencies.wrangler,entrypoint:config.main,admin_auth:true},null,2));
