...
import {cleanupP3Fixtures,createPromoMission,createPromoPost,generatePromoAsset,getAcademyProfile,listPromoAssets,listPromoPosts,promoAssetObject,promoDashboard,saveAcademyProfile} from './promo.js';
...
function securedAppHtml(csrf){const safe=String(csrf||'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));return appHtml().replace('<meta name="csrf-token" content="">','<meta name="csrf-token" content="'+safe+'">');}

export default {async fetch(request,env){
  try{
    const url=new URL(request.url), path=url.pathname;
    if((path==='/health'||path==='/healthz')&&request.method==='GET')return json({ok:true,service:'ai-director-office',version:env.APP_VERSION||'0.5.0',db:Boolean(env.DB),ai:Boolean(env.AI),r2:Boolean(env.PROMO_ASSETS),auth:isAuthConfigured(env),timestamp:nowIso()});
    if(path==='/preflight'&&request.method==='GET'){const ok=Boolean(env.DB)&&Boolean(env.AI)&&Boolean(env.PROMO_ASSETS)&&isAuthConfigured(env);return json({ok,checks:{worker:true,d1:Boolean(env.DB),workersAI:Boolean(env.AI),r2:Boolean(env.PROMO_ASSETS),adminAuth:isAuthConfigured(env)},version:env.APP_VERSION||'0.5.0'},ok?200:503);}
    ...
    if(path==='/'&&request.method==='GET'){const state=await authState(request,env);if(!state.configured)return new Response(loginHtml(false),{status:503,headers:HTML_HEADERS});if(!state.authenticated)return new Response(loginHtml(true),{status:200,headers:HTML_HEADERS});return new Response(securedAppHtml(state.csrf),{status:200,headers:APP_HTML_HEADERS});}
    ...
    let m=match(path,/^\/api\/students\/([^/]+)$/); if(m&&request.method==='GET')return json({ok:true,data:await studentCard(env,decodeURIComponent(m[1]))});
    if(path==='/api/students'&&request.method==='POST')return json({ok:true,data:await createStudent(env,request)},201);
    m=match(path,/^\/api\/students\/([^/]+)\/metrics$/); if(m&&request.method==='POST')return json({ok:true,data:await addMetric(env,request,decodeURIComponent(m[1]))},201);
    ...
    m=match(path,/^\/api\/leads\/([^/]+)\/stage$/); if(m&&request.method==='POST')return json({ok:true,data:await updateLeadStage(env,request,decodeURIComponent(m[1]))});
    ...
    if(path==='/api/promo/dashboard'&&request.method==='GET')return json({ok:true,data:await promoDashboard(env)});
    if(path==='/api/promo/profile'&&request.method==='GET')return json({ok:true,data:await getAcademyProfile(env)});
    if(path==='/api/promo/profile'&&request.method==='POST')return json({ok:true,data:await saveAcademyProfile(env,request)});
    if(path==='/api/promo/mission'&&request.method==='POST')return json({ok:true,data:await createPromoMission(env,await bodyJson(request))},201);
    if(path==='/api/promo/assets'&&request.method==='GET')return json({ok:true,data:await listPromoAssets(env)});
    if(path==='/api/promo/assets/generate'&&request.method==='POST')return json({ok:true,data:await generatePromoAsset(env,await bodyJson(request))},201);
    m=match(path,/^\/api\/promo\/assets\/([^/]+)$/); if(m&&request.method==='GET')return await promoAssetObject(env,decodeURIComponent(m[1]));
    if(path==='/api/promo/posts'&&request.method==='GET')return json({ok:true,data:await listPromoPosts(env)});
    if(path==='/api/promo/posts'&&request.method==='POST')return json({ok:true,data:await createPromoPost(env,await bodyJson(request))},201);
    if(path==='/api/internal/p3-cleanup'&&request.method==='POST')return json({ok:true,data:await cleanupP3Fixtures(env)});
