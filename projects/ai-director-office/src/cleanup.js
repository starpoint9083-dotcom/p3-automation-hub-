...
export async function cleanupP3Fixtures(env){
  ...
  for(const m of missions){await env.DB.prepare(`DELETE FROM promo_posts WHERE mission_id=? AND academy_id=?`).bind(m.id,academy).run();await env.DB.prepare(`DELETE FROM promo_missions WHERE id=? AND academy_id=?`).bind(m.id,academy).run();}
  ...
  return {ok:true,deleted_assets:assets.length,deleted_missions:missions.length};
}
