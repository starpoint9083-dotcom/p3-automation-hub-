...
async function withTimeout(promise,ms,label){
  ...
  try{
    return await Promise.race([
      promise,
      new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error(`${label} ${Math.round(ms/1000)}초 제한 초과`)),ms);})
    ]);
...
export async function generateOrReusePromoImage(env,payload){
  ...
  const req={
    ...
    purpose:String(payload.purpose||'홍보 이미지'),
    channel:String(payload.channel||'GENERAL'),
    width:Math.max(256,Math.min(1920,Math.floor(Number(payload.width)||1080))),
    height:Math.max(256,Math.min(1920,Math.floor(Number(payload.height)||1350))),
    target_segment:String(payload.target_segment||''),
    prompt_hint:String(payload.prompt_hint||'').slice(0,800)
  };
  ...
  form.append('prompt',prompt);form.append('width',String(req.width));form.append('height',String(req.height));
  ...
  const bytes=decodeBase64(image),id=uid('asset'),date=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Seoul',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
  const key=`promo/${academyId(env)}/${date}/${id}.png`;
...
export async function createPromoProductionRun(env,{mission_id,items=[]}){
  const missionId=String(mission_id||'');
  if(!missionId)throw new HttpError(400,'MISSION_ID_REQUIRED');
  const mission=await env.DB.prepare(`SELECT id FROM promo_missions WHERE id=? AND academy_id=?`).bind(missionId,academyId(env)).first();
  if(!mission)throw new HttpError(404,'PROMO_MISSION_NOT_FOUND');
  const runId=uid('prun');
  const normalized=(Array.isArray(items)?items:[]).slice(0,40);
  ...
  for(const item of normalized){
    await env.DB.prepare(`INSERT INTO promo_production_items (id,run_id,academy_id,mission_id,item_type,channel,purpose,payload_json,status) VALUES (?,?,?,?,?,?,?,?,'QUEUED')`).bind(uid('pitem'),runId,academyId(env),missionId,String(item.item_type||'POST'),String(item.channel||''),String(item.purpose||''),JSON.stringify(item.payload||{})).run();
  }
  return promoProductionRun(env,runId);
}

export async function promoProductionRun(env,runId){
  const run=await env.DB.prepare(`SELECT * FROM promo_production_runs WHERE id=? AND academy_id=?`).bind(String(runId),academyId(env)).first();
  if(!run)throw new HttpError(404,'PROMO_RUN_NOT_FOUND');
  const {results:items=[]}=await env.DB.prepare(`SELECT id,item_type,channel,purpose,status,result_ref,attempt_count,last_error,last_retry_at,next_retry_at,created_at,updated_at FROM promo_production_items WHERE run_id=? ORDER BY created_at,id`).bind(run.id).all();
  return {...run,items};
}

export async function recoverPromoProductionRun(env,runId){
  const run=await env.DB.prepare(`SELECT id FROM promo_production_runs WHERE id=? AND academy_id=?`).bind(String(runId),academyId(env)).first();
  if(!run)throw new HttpError(404,'PROMO_RUN_NOT_FOUND');
  const restored=await env.DB.prepare(`UPDATE promo_production_items SET status='QUEUED',last_error='',next_retry_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE run_id=? AND status IN ('PROCESSING','FAILED') AND attempt_count<4`).bind(run.id).run();
