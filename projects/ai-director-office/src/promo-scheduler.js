...
export async function processPendingPromoRuns(env,{max_runs=3,max_items_per_run=1}={}){
  ...
  const {results:runs=[]}=await env.DB.prepare(`SELECT id,status,updated_at FROM promo_production_runs WHERE academy_id=? AND status IN ('QUEUED','RUNNING','PAUSED') ORDER BY CASE status WHEN 'RUNNING' THEN 0 WHEN 'QUEUED' THEN 1 ELSE 2 END, updated_at, created_at LIMIT ?`).bind(academyId(env),limit).all();
  const results=[];
