...
function assetNeedsFor(channels,target){
  ...
  if(channels.includes('YOUTUBE'))needs.push(
    {kind:'IMAGE',channel:'YOUTUBE',purpose:'쇼츠 커버',width:1080,height:1920,required:true,prompt_hint:`${target} 학부모 대상 영어학원 유튜브 쇼츠 세로 커버. 강한 첫인상과 충분한 자막 여백`},
    {kind:'IMAGE',channel:'YOUTUBE',purpose:'쇼츠 장면 1',width:1080,height:1920,required:true,prompt_hint:'영어 교재와 노트가 있는 실제감 있는 학습 장면. 학생 얼굴 식별 없이 자연스러운 세로 구도'},
    {kind:'IMAGE',channel:'YOUTUBE',purpose:'쇼츠 장면 2',width:1080,height:1920,required:false,prompt_hint:'학부모 상담과 아이 학습 성장을 연상시키는 밝고 전문적인 영어학원 세로 비주얼'}
  );
  ...
  if(channels.includes('INSTAGRAM'))needs.push(
    {kind:'IMAGE',channel:'INSTAGRAM',purpose:'피드 커버',width:1080,height:1350,required:true,prompt_hint:`${target} 모집용 프리미엄 영어학원 소셜 미디어 커버. 학부모 대상 신뢰감 있는 교육 분위기`},
    {kind:'IMAGE',channel:'INSTAGRAM',purpose:'릴스 세로 커버',width:1080,height:1920,required:true,prompt_hint:'모바일 릴스용 세로형 영어학원 홍보 비주얼, 자연광, 전문적이고 현대적인 교육 공간'}
  );
...
export async function createPromoMission(env,payload){
  ...
  const system=`당신은 한국 소규모 영어학원의 능동적인 AI 모집·홍보실장이다. 빈자리, 문의, 상담, 등록성과를 보고 이번 주 필요한 홍보물을 먼저 결정하고 제작엔진에 작업을 지시한다. 뮤엠영어·노피곰의 공식 교육내용을 임의로 지어내지 않는다. 성적보장·과장·불안조장·가짜 후기·가짜 희소성을 금지한다. 조회수보다 상담과 신규등록을 우선한다. 유튜브 쇼츠, 네이버 블로그, 인스타그램, 당근은 같은 문구를 복붙하지 않고 각각 영상발견·검색·시각관심·지역신뢰 역할로 분리한다. 부족한 학원 자료가 있으면 원장에게 구체적으로 요청한다.`;
  const user=`신규등록 목표 ${goal}명. 우선 모집대상 ${target}. 빈자리 ${JSON.stringify(open.slice(0,8))}. 기존 유입성과 ${JSON.stringify((perf.source_funnel||[]).slice(0,8))}. 사용 채널 ${JSON.stringify(channels.map(c=>CHANNEL_LABEL[c]))}. 학원 프로필 ${JSON.stringify(profile)}. 부족한 정보 ${JSON.stringify(missing)}. 필요한 자산 ${JSON.stringify(needs)}. 다음 구조로 실행 가능한 한국어 작전을 작성하라: [이번 주 목표] [우선 대상 선정 이유] [이번 주 편성표-유튜브/블로그/인스타/당근 각각 몇 개] [오늘 만들 것] [필요 이미지·영상·자료] [원장님에게 필요한 자료] [게시 순서] [1일/3일/7일 후속] [문의→상담→등록 KPI].`;
...
function channelRules(channel){
  if(channel==='YOUTUBE')return `유튜브 쇼츠 완성 제작패키지. ① 제목 후보 5개 ② 첫 1~2초 훅 3개 ③ 45~60초 타임코드 대본 ④ 6~10개 장면표(초 단위, 화면, 내레이션, 자막) ⑤ 썸네일 문구 3개 ⑥ 설명문 ⑦ 해시태그 8~12개 ⑧ 상담 CTA ⑨ 촬영 또는 AI로 추가 제작해야 할 장면 목록. 첫 3초 안에 학부모 고민을 명확히 잡고 과장된 성적 약속을 하지 않는다.`;
  ...
  if(channel==='INSTAGRAM')return `인스타그램 릴스+피드 통합패키지. 릴스 첫 1~2초 훅, 30~45초 대본, 장면 5~8개, 피드/카드 1~4 한 줄 문구, 캡션 500~900자, DM/프로필 상담 CTA, 해시태그 8~15개를 제공한다. 억지 감성문구와 과한 이모지를 피한다.`;
...
export async function createPromoPost(env,payload){
  ...
  return {id,mission_id:missionId,channel,channel_label:CHANNEL_LABEL[channel],format,title,body,asset_ids:assetIds,status:'READY',publish_mode:['YOUTUBE','INSTAGRAM'].includes(channel)?'CONNECTOR_READY':'APPROVAL_PACKAGE'};
}
