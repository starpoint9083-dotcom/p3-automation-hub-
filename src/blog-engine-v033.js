...
const QUALITY_GATE='v0.3.3-starpoint-v13b-voice';
...
const VOICE_RULES=[
  '결론을 먼저 말한다.',
  '바로 이어 실제 생활 장면이나 고객이 겪는 상황을 보여준다.',
  '주의할 점이나 한계를 숨기지 않는다.',
  '마지막에는 왜 그런지 이유나 근거를 붙인다.',
  '짧고 단단한 생활 언어를 쓴다. 감정 과잉과 광고 문구는 빼고 실제 안경사가 설명하는 느낌으로 쓴다.',
  '“중요합니다”, “추천드립니다”, “최적의 선택”, “전문가와 상담하세요” 같은 AI식 상투 표현을 반복하지 않는다.'
];
...
function styleCount(text){return AIISH.reduce((n,p)=>n+(text.split(p).length-1),0);}
function textIssue(s,min=1,max=5000){
  ...
  if(styleCount(text)>1) return 'ai_style_repetition';
  return null;
}
...
function safeIntro(topic){
  const t=norm(`${topic.keyword} ${topic.blog_bridge}`);
  if(t.includes('자외선')||t.includes('변색')) return '변색렌즈는 야외를 자주 오가는 분에게 편한 선택이 될 수 있습니다. 다만 계절 이름만 보고 고르면 안 됩니다. 실제로 햇빛을 얼마나 자주 받고, 실내와 실외를 얼마나 오가는지, 운전을 많이 하는지를 먼저 봐야 합니다. 같은 변색렌즈라도 생활 패턴에 따라 만족도가 달라지는 이유가 여기 있습니다.';
  return `${topic.keyword}이 신경 쓰인다면 제품부터 고르지 않는 게 좋습니다. 먼저 언제, 어느 거리에서, 어떤 상황이 불편한지부터 봐야 합니다. 그다음 ${topic.blog_bridge}가 그 불편과 맞는지 확인하면 선택이 훨씬 쉬워집니다.`;
}
function buildSafePlan(topic){
  ...
  return {
    ...
    section_plans:[
      {heading:'먼저, 언제 불편한지부터 봅니다',angle:'결론을 먼저 말하고 고객이 실제로 겪는 불편 장면을 보여준다.',key_points:['야외·실내 이동 등 실제 사용 장면','제품보다 불편 상황을 먼저 확인']},
      {heading:'기능은 좋지만, 한계도 같이 봐야 합니다',angle:'렌즈 기능의 장점과 한계를 숨기지 않고 설명한 뒤 이유를 붙인다.',key_points:isPhoto?['실외에서의 변색 특성','차량 안에서는 제품별 반응 차이 확인']:['기능이 도움 되는 사용 환경','도수·제품·환경에 따른 차이']},
      {heading:'내 생활에 맞는지는 여기서 갈립니다',angle:'선택 기준을 결론부터 제시하고 생활 예시로 설명한다.',key_points:['주 사용 장소와 시간','색상·도수·착용 습관 등 개인 조건']},
      {heading:'마지막은 시력검사에서 확인합니다',angle:'검사와 상담에서 무엇을 확인하는지 현실적인 이유와 함께 설명한다.',key_points:['현재 도수와 기존 안경 불편 원인','생활 패턴을 반영한 렌즈 선택']}
    ],
    photo_slots:[
      ...
      {position:'3번째 섹션 뒤',description:'고객 생활패턴을 보여주는 사용 장면',source_preference:'ai',search_query:`${topic.keyword} 생활 장면`},
      {position:'마무리 전',description:'가공·피팅 또는 완성 안경 디테일',source_preference:'owned',search_query:''}
    ],
    video_plan:{hook:`${topic.keyword}, 제품보다 먼저 봐야 할 게 있습니다`,shots:['실제 불편 상황','렌즈 디테일','주의할 한계','검사·상담 장면'],duration_sec:30,caption:'정확한 시력검사, 편안한 안경'},
    hashtags:['#스타포인트안경원','#광안동안경원','#수영구안경원',`#${String(topic.keyword).replace(/\s+/g,'')}`],
    cta:'안경은 기능 이름보다 내 생활에 맞는지가 먼저입니다. 지금 쓰는 안경이 왜 불편한지부터 확인해 보세요.'
  };
}
...
async function section(env,topic,sectionPlan,index,avoidText=''){
  ...
  for(let attempt=1;attempt<=3;attempt++){
    const prompt=[
      ...
      '그 뒤 실제 생활 장면이나 고객이 느끼는 상황을 하나 넣는다.',
      '한계나 주의점을 숨기지 말고, 마지막에는 왜 그런지 설명한다.',
    ...
    try{
      const r=await env.AI.run(WRITER_MODEL,{messages:[
        {role:'system',content:'Write like an experienced Korean optician speaking to a customer. Lead with the conclusion, then a real-life scene, then caution, then the reason. Short firm sentences. No generic AI marketing language. Plain Korean text only.'},
        {role:'user',content:prompt}
      ],temperature:attempt===1?0.28:0.1,max_tokens:760});
...
async function build(env,topic){
  ...
  const draft={mode:'ai-split-writing-starpoint-v13b',title:plan.title,intro:plan.intro,sections,photo_slots:plan.photo_slots,video_plan:plan.video_plan,hashtags:plan.hashtags,cta:plan.cta,generation_meta:{structured_output:true,text_quality_gate_passed:true,quality_gate:QUALITY_GATE,voice_profile:'stella-v13b-starpoint',writer_model:WRITER_MODEL,plan_mode:'deterministic_safe',section_attempts:generated.reduce((s,x)=>s+x.attempts,0),parallel_sections:true,distinct_repairs:repairs,section_lengths:generated.map(x=>x.length),max_section_similarity:Number(maxSim.toFixed(3))}};
...
export default{
  async fetch(request,env,ctx){
    ...
    if(request.method==='GET'&&u.pathname==='/health') return J({ok:true,service:'p3-blog-engine',version:BLOG_ENGINE_VERSION,ai_bound:Boolean(env?.AI),structured_generation:true,text_quality_gate:QUALITY_GATE,voice_profile:'stella-v13b-starpoint',live_trend_source:'Google Trends KR RSS',media_policy:MEDIA_POLICY});
