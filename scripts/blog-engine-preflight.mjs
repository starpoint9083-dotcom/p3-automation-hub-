...
for (const marker of [
  ...
  if (!source.includes(marker)) throw new Error(`missing_v0342_marker:${marker}`);
}
...
if (!source.includes("const QUALITY_GATE='v0.3.4.2-starpoint-natural-friendly'")) throw new Error('voice_quality_gate_missing');
if (source.includes("return 'friendly_tone_missing'")) throw new Error('per_section_friendly_requirement_still_present');

const config = fs.readFileSync('src/blog-engine-config.js','utf8');
if (!config.includes("BLOG_ENGINE_VERSION = '0.3.4.2'")) throw new Error('wrong_blog_engine_version');
...
console.log('BLOG_ENGINE_V0342_PREFLIGHT_OK');