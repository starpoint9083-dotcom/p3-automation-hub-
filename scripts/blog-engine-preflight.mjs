...
for (const marker of [
  ...
  'VOICE_RULES','AIISH','RIGID','FRIENDLY_ENDING','FRIENDLY_GLOBAL','friendlyCount',
  ...
  if (!source.includes(marker)) throw new Error(`missing_v0341_marker:${marker}`);
}
...
if (!source.includes("const QUALITY_GATE='v0.3.4.1-starpoint-friendly-precision'")) throw new Error('voice_quality_gate_missing');

const config = fs.readFileSync('src/blog-engine-config.js','utf8');
if (!config.includes("BLOG_ENGINE_VERSION = '0.3.4.1'")) throw new Error('wrong_blog_engine_version');
...
console.log('BLOG_ENGINE_V0341_PREFLIGHT_OK');