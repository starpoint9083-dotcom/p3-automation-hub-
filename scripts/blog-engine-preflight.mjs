...
const required = ['src/blog-engine.js','src/blog-engine-v032.js','src/blog-engine-config.js','wrangler.blog-engine.jsonc'];
for (const file of required) {
  if (!fs.existsSync(file)) throw new Error(`missing:${file}`);
}

const source = fs.readFileSync('src/blog-engine-v032.js','utf8');
for (const marker of [
  '/api/draft','WRITER_MODEL','buildSafePlan','safeIntro','BAD_PATTERNS','BAD_FOREIGN',
  'text_quality_gate_passed','distinct_repairs','max_section_similarity','Promise.all',
  "plan_mode:'deterministic_safe'",'section_attempts'
]) {
  if (!source.includes(marker)) throw new Error(`missing_v032_marker:${marker}`);
}
...
if (!config.includes("BLOG_ENGINE_VERSION = '0.3.2'")) throw new Error('wrong_blog_engine_version');
...
if (!wrangler.includes('"main": "src/blog-engine-v032.js"')) throw new Error('wrong_worker_entry');
if (!wrangler.includes('"binding": "AI"')) throw new Error('workers_ai_binding_missing');

console.log('BLOG_ENGINE_V032_PREFLIGHT_OK');
