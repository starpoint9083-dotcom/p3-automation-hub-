...
await check("/preflight", b => b?.ok === true && b?.checks?.workerRuntime === true && b?.checks?.p1BridgeConfigured === true && b?.checks?.p2SupervisorConfigured === true && b?.checks?.p2TechnicalQualityGateConfigured === true && b?.checks?.p2MotionQualityObservationConfigured === true && b?.safety?.paidCinemaGenerationFromP3 === false && b?.safety?.paidVisualAIFromP3 === false && b?.safety?.paidMotionAIFromP3 === false);
...
await check("/projects/p2", b => b?.ok === true && b?.project === "P2" && b?.bridge === "configured" && b?.control?.mode === "read-only-supervisor" && b?.probes?.quality === "/api/cinema/batch/quality-public" && b?.probes?.motionQc === "/api/cinema/batch/motion-qc/latest-public");
...
console.log(`E2E COMPLETE ${base} P1_BRIDGE=CONNECTED P2_SUPERVISOR=CONNECTED P2_TECHNICAL_QC=CONNECTED P2_VISUAL_QC=OBSERVED P2_MOTION_QC=OBSERVED PAID_CINEMA_TRIGGER=DISABLED PAID_VISUAL_AI_TRIGGER=DISABLED PAID_MOTION_AI_TRIGGER=DISABLED`);
